...
ap.add_argument("-m", "--min-area", type=int, default=900,
# ap.add_argument("-m", "--min-area", type=int, default=1500, 
        help="Minimum area in pixels difference to be considered actual motion.")
ap.add_argument("-t", "--thresh", default=150, type=int,
...
while True:
   ...
   # blurred_frame = cv2.GaussianBlur(grayscale_frame, (21, 21), 0)
    blurred_frame = cv2.createBackgroundSubtractorMOG2(300, 400, True)
    ...
    thresholded_frame = cv2.threshold(frame_delta, args["thresh"], 220, cv2.THRESH_BINARY)[1]
    ...
    font_color = (0,220, 0) if status == 'Area vacant.' else (0, 0, 220)
